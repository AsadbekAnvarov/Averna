...
import type { Part1Topic, Part2Card, Part3Question } from "@/lib/speaking-data";
...
interface SpeakingRunnerProps {
  ...
  todayTopic: SpeakingTopic;
}

export function SpeakingRunner({ part1Topics, part2Cards, part3Questions, todayTopic }: SpeakingRunnerProps) {
  ...
  const currentTopic = part1Topics[p1Topic];
  const currentP1Q = currentTopic.questions[p1Q % currentTopic.questions.length];
  const currentP2 = part2Cards[p2];
  const currentP3 = part3Questions[p3];
  ...
  const newCue = () => {
    stopPart2();
    setP2Reveal(false);
    setP2((i) => (i + 1) % part2Cards.length);
  };
  const nextP3 = () => {
    setP3Reveal(false);
    setP3((i) => (i + 1) % part3Questions.length);
  };
