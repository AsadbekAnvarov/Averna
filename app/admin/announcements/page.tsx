    ...
    <div className="min-h-screen premium-gradient">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <PageHeader
          ...
          title={<>Umumiy <span className="neon-text">eʼlonlar</span></>}
          subtitle="Markazdagi hammaga — barcha oʻquvchilar va oʻqituvchilarga xabar yuboring."
        />

        {searchParams.saved && <div className="mb-6 p-3 rounded-lg bg-averna-neon/10 border border-averna-neon/30 text-averna-neon">✓ Hammaga yuborildi va bildirishnoma joʻnatildi!</div>}

        <Card className="glass border-averna-neon/30 mb-8">
          <CardHeader><CardTitle className="flex items-center gap-2 text-averna-neon"><Send className="h-5 w-5" /> Yangi umumiy eʼlon</CardTitle></CardHeader>
          <CardContent>
            <form action={postGlobal} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="title">Sarlavha</Label>
                <Input id="title" name="title" placeholder="masalan, Bayram kuni markaz yopiq" className="bg-background/50" required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="body">Xabar</Label>
                <textarea id="body" name="body" rows={4} placeholder="Hamma uchun eʼloningizni yozing..."
              ...
              <Button type="submit" className="w-full neon-button bg-averna-primary hover:bg-averna-light">Hammaga yuborish</Button>
            </form>
          </CardContent>
        </Card>

        <Card className="glass border-white/10">
          <CardHeader><CardTitle className="text-white">Soʻnggi eʼlonlar</CardTitle></CardHeader>
          <CardContent>
            {recent.length === 0 ? (
              <p className="text-gray-400 text-sm">Hozircha umumiy eʼlonlar yoʻq.</p>
            ) : (
              <div className="space-y-3">
                {recent.map((a) => (
                  <div key={a.id} className="p-3 rounded-lg bg-white/5 border border-white/10">
                    ...
                    <p className="text-[11px] text-gray-500 mt-1">muallif: {a.authorName} · {formatDateTime(a.createdAt)}</p>
                  </div>
                ))}
              </div>
            )}
